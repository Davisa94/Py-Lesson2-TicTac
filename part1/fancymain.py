...
# The main game loop that controls all game actions
def game_loop():
    coords = ""
    players = ["X", "O"]
    moves = []
...
#Given the raw game board state print out the game board 
...
def raw_move_to_fancy_board(game_state):
...
def fancy_draw(game_state):
    ...
    # We fill any gaps with #
    filled_moves = fill_gaps(ordered_moves)
    fancy_moves = raw_move_to_fancy_board(filled_moves)
